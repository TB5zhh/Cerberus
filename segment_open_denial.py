...
from palette import PALETTE, convert_rgb_to_mask
...
class DRNSeg(nn.Module):
    def __init__(self, model_name, classes, pretrained_model=None,
        ...
        self.fc2 = nn.Conv2d(512 , classes , kernel_size = 1 , bias = False)
...
class EmulatedMS(torch.utils.data.Dataset):
    def __init__(self, data_dir, phase, transforms, scales, list_dir=None, series=22, cut=5):
        self.img_dir = f"{data_dir}/{series}/rgb"
        self.label_dir = f"{data_dir}/{series}/mask"
        self.cut = 5
        self.transforms = transforms
        self.scales = scales
        with open(f'{data_dir}/{series}/config.json') as f:
            config = json.load(f)
        self.count = min([config['size'], len(os.listdir(self.img_dir)) - cut, len(os.listdir(self.label_dir))])
        assert self.count > 0, 'Invalid cut size'
            
    
    def __getitem__(self, index):
        data = [
            Image.open(f"{self.img_dir}/{index + self.cut}.png"),
            # Image.fromarray(np.asarray(convert_rgb_to_mask(torch.as_tensor(np.asarray(Image.open(f"datasets/emulated/22/mask_rgb/{i}")))))).save(f"datasets/emulated/22/mask/{i}")
            Image.open(f"{self.label_dir}/{index + self.cut}.png"),
        ]
        w, h = data[0].size
        ...
        out_data.append(f"{self.img_dir}/{index + self.cut}.png")
...
def validate(val_loader, model, criterion, eval_score=None, print_freq=10, text_features = None, deny_text_features = None):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            deny_text_features = deny_text_features.to(torch.float32)
            
            result = torch.einsum('abcd,eb->aecd' , (features,text_features))
            deny_result = torch.einsum('abcd,eb->aecd' , (features,deny_text_features))
            
            tempsoftmax = nn.LogSoftmax(dim=1)
            real_target = torch.nn.functional.one_hot(target_var.flatten()[target_var.flatten() != 255], num_classes=19).flatten()
            real_result_1 = result.permute(0,2,3,1).reshape(-1,19)[target_var.flatten() != 255, :].flatten()
            real_result_2 = deny_result.permute(0,2,3,1).reshape(-1,19)[target_var.flatten() != 255, :].flatten()
            real_result = torch.stack((real_result_2, real_result_1), 1)
            real_result = tempsoftmax(real_result)
            loss = criterion(real_result, real_target) 
            
            # relative_result = tempsoftmax(result - deny_result)
            # loss = criterion(relative_result, target_var)
...
def train(train_loader, model, criterion, optimizer, epoch,
          eval_score=None, print_freq=1, text_features = None, deny_text_features = None):
    batch_time = AverageMeter()
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        real_result = torch.stack((real_result_2, real_result_1), 1)
        real_result = tempsoftmax(real_result)
        if real_result.shape[0] != real_target.shape[0]:
            embed()
        loss = criterion(real_result, real_target) 
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, 'model_best_tbw.pth.tar')
...
def train_seg(args):
    ...
    voc = ["road" , "sidewalk" , "building" , "wall" , "fence" , "pole" , "traffic light" , "traffic sign" , "vegetation" , "terrain" , "sky" , "person" , "rider" , "car" , "truck" , "bus" , "train" , "motorcycle" , "bicycle"]
    deny_voc = [f"not {i}" for i in voc]
    
    prompt = clip.tokenize(voc).cuda()
    deny_prompt = clip.tokenize(deny_voc).cuda()
    
    text_features = lmodel.encode_text(prompt).detach()
    deny_text_features = lmodel.encode_text(deny_prompt).detach()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train(train_loader, model, criterion, optimizer, epoch,
              eval_score=accuracy, text_features = text_features, deny_text_features = deny_text_features)

        # evaluate on validation set
        prec1 = validate(val_loader, model, criterion, eval_score=accuracy, text_features = text_features, deny_text_features=deny_text_features)

        is_best = prec1 > best_prec1
        best_prec1 = max(prec1, best_prec1)
        checkpoint_path = 'checkpoint_latest_tbw.pth.tar'
        save_checkpoint({
            'epoch': epoch + 1,
            'arch': args.arch,
        ...
        if (epoch + 1) % 10 == 0:
            history_path = 'checkpoint_{:03d}_tbw.pth.tar'.format(epoch + 1)
...
def test_ms(eval_data_loader, model, num_classes, scales,
            output_dir='pred', has_gt=True, save_vis=False, text_features = None, deny_text_features = None):
    model.eval()
    batch_time = AverageMeter()
    data_time = AverageMeter()
    end = time.time()
    ...
    for iter, input_data in enumerate(eval_data_loader):
        ...
        with torch.no_grad():
            for image in images:
                ...
                # if deny_text_features is not None:
                deny_text_features = deny_text_features.to(torch.float32)
                deny_result = torch.einsum('abcd,eb->aecd', (features, deny_text_features))
                # final_result = tempsoftmax(result - deny_result)
                # else:
                #     final_result = tempsoftmax(result)
                #real_target = torch.nn.functional.one_hot(label.flatten()[label.flatten() != 255]).flatten()
                # embed()
                real_result_1 = result.permute(0,2,3,1)
                real_result_2 = deny_result.permute(0,2,3,1)
                real_result = torch.stack((real_result_2, real_result_1), 1)
                real_result = tempsoftmax(real_result)
                #loss = criterion(real_result, real_target) 
                final_result = nn.Softmax(dim=1)(torch.stack((result, deny_result), 1))[:, 0]
                outputs.append(final_result.data)
...
def test_seg(args):
    ...
    lmodel, lpreprocess = clip.load("ViT-B/32" , device="cuda")
    voc = ["road" , "sidewalk" , "building" , "wall" , "fence" , "pole" , "traffic light" , "traffic sign" , "vegetation" , "terrain" , "sky" , "person" , "rider" , "car" , "truck" , "bus" , "train" , "motorcycle" , "bicycle" ]
    # voc = ["road" , "sidewalk" , "building" , "wall" , "fence" , "pole" , "traffic light" , "traffic sign" , "vegetation" , "terrain" , "sky" , "person" , "rider" , "car" , "truck" , "bus" , "train" , "motorcycle" , "deer"]
    deny_voc = [f"not {i}" for i in voc]
    prompt = clip.tokenize(voc).cuda()
    deny_prompt = clip.tokenize(deny_voc).cuda()
    text_features = lmodel.encode_text(prompt).detach()
    ...
    if args.ms:
        mAP = test_ms(test_loader, model, args.classes, save_vis=True,
                      ...
                      output_dir=out_dir,
                      scales=scales, text_features = text_features, deny_text_features = deny_text_features)
    else:
