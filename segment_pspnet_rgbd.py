...
class DRNSeg(nn.Module):
    def __init__(self, model_name, classes, pretrained_model=None,
        ...
        model_d = drn.__dict__.get(model_name)(
            pretrained=pretrained, num_classes=1000)
        pmodel = nn.DataParallel(model)
        pmodel_d = nn.DataParallel(model_d)
        if pretrained_model is not None:
            pmodel.load_state_dict(pretrained_model)
        self.base = nn.Sequential(*list(model.children())[:-2])
        self.base_d = nn.Sequential(*list(model_d.children())[:-2])
        ...
        self.pool1_conv = nn.Conv2d(model.out_dim * 2 , 512 , kernel_size = 1 , bias = False)
        self.pool1_bn = nn.BatchNorm2d(512)
        self.pool2_conv = nn.Conv2d(model.out_dim * 2 , 512 , kernel_size = 1 , bias = False)
        self.pool2_bn = nn.BatchNorm2d(512)
        self.pool3_conv = nn.Conv2d(model.out_dim * 2 , 512 , kernel_size = 1 , bias = False)
        self.pool3_bn = nn.BatchNorm2d(512)
        self.pool6_conv = nn.Conv2d(model.out_dim * 2 , 512 , kernel_size = 1 , bias = False)
        self.pool6_bn = nn.BatchNorm2d(512)

        self.fc1 = nn.Conv2d(3072 , 512 , kernel_size = 3 , padding = 1 , bias = False)
    ...
    def optim_base_parameters(self, memo=None):
        for param in self.base.parameters():
            yield param
        for param in self.base_d.parameters():
            yield param
...
class SegList(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.hha_list is not None:
       	    data.append(Image.open(join(self.data_dir, self.hha_list[index])))
        if self.label_list is not None:
       	    data.append(Image.open(join(self.data_dir, self.label_list[index])))
        data = list(self.transforms(*data))
        #sio.savemat('debug.mat' , {'data':data[0].numpy() , 'data2':data[1].numpy()})
        if self.out_name:
    ...
    def read_lists(self):
        ...
        hha_path = join(self.list_dir, self.phase + '_hhas.txt')
        ...
        if exists(hha_path):
            self.hha_list = [line.strip() for line in open(hha_path, 'r')]
            assert len(self.image_list) == len(self.hha_list)
...
class SegListMS(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for s in self.scales:
            img, hha = self.transforms(data[0].resize((int(w * s), int(h * s)),Image.BICUBIC) , data[1].resize((int(w * s) , int(h*s)), Image.BICUBIC))
            out_data.append(img)
            out_data.append(hha)
        #ms_images = [self.transforms(data[0].resize((int(w * s), int(h * s)),Image.BICUBIC) , data[1].resize((int(w * s) , int(h*s)) , Image.BICUBIC))[0:1] for s in self.scales]
...
def validate(val_loader, model, criterion, eval_score=None, print_freq=10):
    ...
    for i, (input, hha, target) in enumerate(val_loader):
    
        small_target = torch.zeros(target.size(0) , int(target.size(1)/8) , int(target.size(2)/8))
        ...
        with torch.no_grad():
            ...
            hha_var = torch.autograd.Variable(hha, volatile=True)
            target_var = torch.autograd.Variable(target, volatile=True)

            # compute output
            output = model(input_var , hha_var)[0]
...
def train(train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (input, hha, target) in enumerate(train_loader):
        ...
        hha_var = torch.autograd.Variable(hha)
...
def test_ms(eval_data_loader, model, num_classes, scales,
    ...
    for iter, input_data in enumerate(eval_data_loader):
        ...
        if has_gt:
            name = input_data[3]
            label = input_data[2]
            images.extend(input_data[4::2])
            hhas.extend(input_data[5::2])
        else:
            name = input_data[2]
            images.extend(input_data[3::2])
            hhas.extend(input_data[4::2])
        # pdb.set_trace()
        outputs = []
        #for image in images:
        with torch.no_grad():
            for i in range(0,len(images)):
                image_var = Variable(images[i], requires_grad=False, volatile=True)
                hha_var = Variable(hhas[i], requires_grad=False, volatile=True)
                final = model(image_var , hha_var)[0]
